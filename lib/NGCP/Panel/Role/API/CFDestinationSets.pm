    ...
    for my $d (@{ $resource->{destinations} }) {
        ...
        #todo: I think that user expects that put and get will be the same
            if(('customhours' ne $d->{destination}) && ('sip:custom-hours@app.local' ne $d->{destination}) ){
                $c->log->error("Invalid paramster 'announcement_id' for the destination '".$d->{destination}."'");
                $self->error($c, HTTP_UNPROCESSABLE_ENTITY, "Invalid paramster 'announcement_id' for the destination '".$d->{destination}."'");
                return;
