...
sub insert {
    ...
    my $prov_subscriber = $subscriber->provisioning_subscriber;
    ...
    save_subscriber_profile(
        schema => $schema,
        event => $event,
        subscriber_profile => ($prov_subscriber ? $prov_subscriber->subscriber_profile : undef),
    ...
    save_subscriber_profile_set(
        schema => $schema,
        event => $event,
        subscriber_profile_set => ($prov_subscriber ? $prov_subscriber->subscriber_profile_set : undef),
    ...
    if ($bm_actual->billing_mappings->first->product->product_class eq 'pbxaccount') {
        my $pilot_subscriber = $customer->voip_subscribers->search({
        ...
        if ($pilot) {
            my $pilot_prov_subscriber = $pilot_subscriber->provisioning_subscriber;
            save_e164(
                schema => $schema,
                event => $event,
            ...
            save_subscriber_profile(
                schema => $schema,
                event => $event,
                subscriber_profile => ($pilot_prov_subscriber ? $pilot_prov_subscriber->subscriber_profile : undef),
            ...
            save_subscriber_profile_set(
                schema => $schema,
                event => $event,
                subscriber_profile_set => ($pilot_prov_subscriber ? $pilot_prov_subscriber->subscriber_profile_set : undef),
