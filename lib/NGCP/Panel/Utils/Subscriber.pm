...
sub resource_from_item {
    ...
    my ($c, $item, $patch_mode, $validate_code, $get_customer_code) = @params{qw/
        c
        item
        patch_mode
        validate_code
    ...
    my $customer = $get_customer_code
        ? $get_customer_code->($item->contract_id)
        : $item->contract;
    ...
    if ($validate_code) {
        # form validation during PATCH causes fields to be removed from the
        # %resource and then apply_patch() removes the fields that were not a
        # part the PATCH ops from the database, therefore a copy of the resource
        # is validated instead, preserving the original one when $patch_mode is
        # enabled
        my %validate_resource = %resource;
        return unless $validate_code->($patch_mode ? \%validate_resource : \%resource);
    }
    ...
    if ($pref && $pref->first && $pref->first->value) {
    ...
    # ngcp-provisioning-template Catalyst stub has no user(); presume admin.
    my $user = ($c->can('user') ? $c->user : undef);
    my $roles = $user ? $user->roles : 'admin';
    my $show_passwords = ($user && $user->can('show_passwords')) ? $user->show_passwords : 0;

    if ($roles eq "admin" || $roles eq "reseller" ||
        $roles eq "ccareadmin" || $roles eq "ccare") {
        $resource{customer_id} = $contract_id;
        $resource{uuid} = $item->uuid;

        my $lock_pref = NGCP::Panel::Utils::Preferences::get_usr_preference_rs(
            c => $c, attribute => 'lock',
            prov_subscriber => $item->provisioning_voip_subscriber);
        $resource{lock} = 0;
        ...
        if ($show_passwords) {
            foreach my $k (qw/password webpassword/) {
                ...
                if ($@) {
                    $c->log->error("Failed to encrypt $k: " . $@) if $c->can('log');
                    delete $resource{'_' . $k};
                }
            }
        } else {
        ...
        delete $resource{'password'} if $roles eq 'subscriber';

        if ($roles eq "subscriberadmin") {
            $resource{customer_id} = $contract_id;
            if ($user && $item->id != $user->voip_subscriber->id) {
...
sub update_subscriber {
    ...
    my ($c, $schema, $item, $full_resource, $resource, $err_code) = @params{qw/
        c
        schema
        item
        full_resource
    ...
    $schema //= $c->model('DB');
    $resource //= $full_resource->{resource};
    ...
    $resource->{timezone} = $resource->{timezone}->{name} if 'HASH' eq ref $resource->{timezone};
    ...
    if ($subscriber->provisioning_voip_subscriber->is_pbx_pilot && !is_true($resource->{is_pbx_pilot})) {
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "Cannot revoke is_pbx_pilot status from a subscriber.");
        return;
    }

    if ($resource->{customer_id} && ($resource->{customer_id} != $subscriber->contract->id)) {
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "customer_id can't be changed.");
        return;
    }

    if ($resource->{timezone} && !NGCP::Panel::Utils::DateTime::is_valid_timezone_name($resource->{timezone})) {
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "invalid timezone name.");
        return;
    }
    ...
    if ($subscriber->status ne $resource->{status}) {
        if ($resource->{status} eq 'locked') {
            ...
            try {
                terminate(c => $c, subscriber => $subscriber);
                return $subscriber;
            } catch ($e) {
                &$err_code(HTTP_INTERNAL_SERVER_ERROR, "Failed to terminate subscriber",
    ...
    try {
        lock_provisoning_voip_subscriber(
            c => $c,
            prov_subscriber => $prov_subscriber,
            level => $resource->{lock} || 0,
        );
    } catch ($e) {
        &$err_code(HTTP_INTERNAL_SERVER_ERROR, "Failed to update subscriber lock",
                   "failed to lock subscriber id ".$subscriber->id." with level ".$resource->{lock}, $e);
        return;
    }

    my ($error, $profile_set, $profile) = check_profile_set_and_profile($c, $resource, $subscriber);
    if ($error) {
        &$err_code($error->{response_code}, $error->{description}, $error->{error});
        return;
    }
    ...
    try {
        ...
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "Number '" . $1 . "' already exists.", "Number already exists.",
                   "failed to update subscriber, number " . $1 . " already exists");
        return;
    } catch ($e where { /alias \d+ already exists/ }) {
        $e =~ /alias (\d+) already exists/;
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "Number '" . $1 . "' already exists.", "Number already exists.",
                   "failed to update subscriber, alias " . $1 . " already exists");
        return;
    } catch ($e where { /aliases \S+ already exist/ }) {
        $e =~ /aliases (\S+) already exist/;
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "Numbers '" . $1 . "' already exist.", "Numbers already exist.",
                   "failed to update subscriber, aliases " . $1 . " already exist");
        return;
    } catch ($e where { /more than \d+ provided aliases/ }) {
        &$err_code(HTTP_UNPROCESSABLE_ENTITY, "more than 10 provided aliases already exist");
        return;
    }
...
sub update_subscriber_numbers {
    ...
    if ($primary_number && ref $primary_number eq 'HASH' && $primary_number->{cc}) {
        $new_cc = $primary_number->{cc};
        $new_ac = $primary_number->{ac} // '';
        $new_sn = $primary_number->{sn};
        $new_pri_cli = $new_cc . $new_ac . $new_sn;
    }

    my $same_primary_number = defined($old_cc) && defined($new_cc)
        ? ($old_cc eq $new_cc && $old_ac eq $new_ac && $old_sn eq $new_sn)
        : ($old_pri_cli eq $new_pri_cli);
