        ...
        if ( $condition eq 'is' ) {
            foreach my $check_param (keys %{$conditions->{$condition}}) {
                ...
                if ( $check_param =~ /.+\..+/ ) {
                    ...
                    is ($check_value, $conditions->{$condition}->{$check_param}, $test_name);
                }
                elsif ( $check_param eq 'code' ) {
                    is ($result->code, $conditions->{$condition}->{$check_param}, $test_name);
                }
                elsif ( $check_param eq 'header' ) {
                    foreach my $header_condition ( keys %{$conditions->{$condition}->{$check_param}} ) {
                        is ($result->header($header_condition), $conditions->{$condition}->{$check_param}->{$header_condition}, $test_name);
                    }
                }
                else {
                    is ($check_param, $conditions->{$condition}->{$check_param}, $test_name);
                }
            }
        }
            ...
            foreach my $check_param (keys %{$conditions->{$condition}}) {
                ...
                if ( $conditions->{$condition}->{$check_param} eq 'defined' ) {
                    if ( $check_param =~ /.+\..+/ ) {
                        my @splitted_values = split (/\./, $check_param);
                        my $check_value = $self->_retrieve_from_composed_key( $result, \@splitted_values, $retained );
                        ok(defined $check_value, $test_name);
                    }
                }
            }
        }
