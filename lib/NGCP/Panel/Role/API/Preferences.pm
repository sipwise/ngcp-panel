...
sub get_resource {
    ...
    foreach my $pref($prefs->all) {
        ...
        SWITCH: for ($pref->attribute->attribute) {
            ...
            /^cdr_export_sourceclidestinationuserin_rewrite_rule_set_id$/ && do {
...
sub update_item {
    ...
    my $extra = [qw/
        ...
        cdr_export_sourceclidestinationuserin_rewrite_rule_set_id
        ...
        try {
            foreach my $k(keys %{ $old_resource }) {
                SWITCH: for ($k) {
                    ...
                    /^cdr_export_sourceclidestinationuserin_rewrite_rule_set$/ && do {
                        unless(exists $resource->{$k}) {
