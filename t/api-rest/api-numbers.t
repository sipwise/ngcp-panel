...

{
    ...
    $subscriber->{content}->{alias_numbers} = [
        { cc=> '111', ac => '111'.$ticket, sn => $time },
        { cc=> '112', ac => '112'.$ticket, sn => $time },
        { cc=> '113', ac => '112'.$ticket, sn => $time },
    ];
    my ($res,$content,$request) = $subscriber_test_machine->put2get(@{$$subscriber->[1]}{qw/content location/});
}
