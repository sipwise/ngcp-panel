...
sub create_dynamic_preference {
    ...
    my $autoprov_device_id  = delete $resource->{autoprov_device_id};
    ...
    if (defined $resource->{dev_pref} && $resource->{dev_pref} && $autoprov_device_id) {
        $preference->create_related('voip_preference_relations', {
            autoprov_device_id => $autoprov_device_id
        });
    }
    return $preference;
}
