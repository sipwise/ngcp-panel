    ...
    if($posted && $form->validated) {
        ...
        try {
            ...
            NGCP::Panel::Utils::Message::info(
                c => $c,
                data => \%log_data,
                desc => $c->loc("rovisioning template '[_1]' done: subscriber [_2] created", $template, $context->{subscriber}->{username} . '@' . $context->{domain}->{domain}),
            );
        } catch($e) {
            NGCP::Panel::Utils::Message::error(
                c => $c,
...
sub provision_commit_row {
    ...
    try {
        ...
        my ($msg, $subscriber) = _get_duplicate_subs(
            ...
            $schema,
            $e,
            $values->{$PURGE_FIELD_NAME},
        );
        if ($values->{$PURGE_FIELD_NAME}) {
            if ($subscriber) {
...
sub _init_subscriber_preferences_context {
    ...
    if (exists $template->{subscriber_preferences}) {
        ...
        #$subscriber_preferences{create_timestamp} //= $context->{now};
        #$subscriber_preferences{modify_timestamp} //= $context->{now};
...
sub _init_contract_preferences_context {
    ...
    if (exists $template->{contract_preferences}) {
        ...
        #$contract_preferences{create_timestamp} //= $context->{now};
        #$contract_preferences{modify_timestamp} //= $context->{now};
...
sub _init_registrations_context {
    ...
    foreach my $template_registration (@{$template->{registrations} // []}) {
        ...
        #$registration{subscriber_id} //= $context->{subscriber}->{id};
...
sub _init_trusted_sources_context {
    ...
    foreach my $template_trusted_source (@{$template->{trusted_sources} // []}) {
        ...
        #$subscriber{create_timestamp} //= $context->{now};
        #$subscriber{modify_timestamp} //= $context->{now};
