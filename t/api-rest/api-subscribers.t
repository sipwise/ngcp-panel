...
if($pilot->{content}->{total_count} > 0){
    ...
    my($response,$content,$request) = $test_machine->request_patch(  [ { op => 'replace', path => '/alias_numbers', value => [] } ], $pilot->{location} );
    $test_machine->http_code_msg(200, "remove pilot aliases to don't intersect with them", $response,  $content );
    die();
}else{
    undef $pilot;
}
