...
sub get_form {
    my ($self, $c) = @_;
    require NGCP::Panel::Form::Balance::CustomerBalanceAPI;
...
sub hal_from_item {
    ...
    my $hal = NGCP::Panel::Utils::DataHal->new(
        links => [
            ...
            NGCP::Panel::Utils::DataHalLink->new(relation => 'self', href => sprintf("%s%d", $self->dispatch_path, $item->contract->id)),
            NGCP::Panel::Utils::DataHalLink->new(relation => 'ngcp:customers', href => sprintf("/api/customers/%d", $item->contract->id)),
            NGCP::Panel::Utils::DataHalLink->new(relation => 'ngcp:balanceintervals', href => sprintf("/api/balanceintervals/%d/%d", $item->contract->id, $item->id)),
