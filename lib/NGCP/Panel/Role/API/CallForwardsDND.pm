package NGCP::Panel::Role::API::CallForwardsDND;
...
sub _item_rs {
    ...
    $item_rs = $c->model('DB')->resultset('voip_cf_destination_sets')
        ->search(
            { 'provisioning_voip_subscribier.status' => { '!=' => 'terminated' } },
    ...
    if ($c->user->roles eq "reseller") {
        ...
        $item_rs = $item_rs->search({
            'provisioning_voip_subscriber.uuid' => $c->user->uuid,
        });
    }
...
sub update_item {
    ...
    my $allowed_prefs = { cfu => 1, cfb => 1, cft => 1, cfna => 1, cfs => 1 };
    if (!$preference || $preference ne 'internal') {
        $allowed_prefs = NGCP::Panel::Utils::Preferences::get_subscriber_allowed_prefs(
            c => $c,
            prov_subscriber => $prov_subs,
            pref_list => [qw/cfu cfb cft cfna cfs/],
        );
