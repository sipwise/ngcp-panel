...
#See detailed description of the implemented logic in the workfron tasks 37459, 14589 
    ...
    if (!$socket) {
        ...
        $c->log->debug('compose_location_path: socket is empty, return default path:'.$path_default);
        return $path_default;
    }
    ...
    if ($subscriber_lb_ptr_preference_rs && $subscriber_lb_ptr_preference_rs->first) {
        $c->log->debug('compose_location_path: lbrtp_set:'.$subscriber_lb_ptr_preference_rs->first->value);
    ...
    $c->log->debug('compose_location_path: path:'.$path);
    return $path;
}
