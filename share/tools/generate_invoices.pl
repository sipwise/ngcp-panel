        ...
        foreach my $client_contact (@{ get_provider_clients_contacts($provider_contract) } ){
            ...
            foreach my $client_contract(@{ get_client_contracts($client_contact) }){
                ...
                if(!$opt->{sendonly}){
                    ...
                    $invoices->{$client_contract->{id}} = $dbh->selectall_arrayref('select invoices.id from billing.invoices where invoices.generator="auto" 
                    '.ifp(' and ',
                        join(' and ',
...
sub generate_invoice_data{
    ...
    $dbh->do('update billing.invoices set sent_date=?,data=?,amount_net=?,amount_vat=?,amount_total=? where id=?',undef,undef,@$invoice{qw/data amount_net amount_vat amount_total id/});
    return $invoice;
}

sub get_contract_balance{
    my($client_contract,$billing_profile,$contract_balance,$contract_balance_data,$invoice,$stime,$etime) = @_;
    if(!($contract_balance = $dbh->selectrow_hashref('select * from billing.contract_balances where contract_id=? and date(start)=? and date(end)=?',undef,$client_contract->{id},$stime->ymd,$etime->ymd))){
        ...
        #$contract_balance = $dbh->selectrow_hashref('select * from billing.contract_balances where id=?',undef,);
        $contract_balance = $dbh->selectrow_hashref('select * from billing.contract_balances where contract_id=? and date(start)=? and date(end)=?',undef,$client_contract->{id},$stime->ymd,$etime->ymd);
        #$logger->debug( Dumper $contract_balance );
    }else{
...
#            print Dumper $contract_balance_data;
            ...
            if(my $sql_update = join(',',@sql_update)){
                $dbh->do('update billing.contract_balances set '.$sql_update.' where contract_id=? and date(start)=? and date(end)=?',undef,@sql_update_params,$client_contract->{id},$stime->ymd,$etime->ymd);
            }
        }
...
#todo: repeat my old function based on templates and store into utils
    ...
    if($client_contact->{email}){
        ...
        $dbh->do('update billing.invoices set sent_date=now() where '.ify( ' id ',  @invoice_ids ), undef, @invoice_ids);
    }#we have correct "To"
}
