...
sub _item_rs {
    ...
    foreach my $type (qw(call voicemail sms fax xmpp)) {
        ...
        if ((not defined $type_param) or index(lc($type_param),$type) > -1) {
            my $sub_name = 'get_' . $type . '_rs';
            my $rs = $self->$sub_name($c,$uuid,$contract_id,$reseller_id,$provider_id);
...
sub get_call_rs {
    ...
    my $self = shift;
    my %params = @_;
    my ($c,$uuid,$contract_id,$provider_id) = @params{qw/c uuid contract_id provider_id/};
...
sub get_voicemail_rs {
    
    my $self = shift;
    my %params = @_;
    my ($c,$uuid,$contract_id,$reseller_id) = @params{qw/c uuid contract_id reseller_id/};
