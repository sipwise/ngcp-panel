...
has_field 'billing_profile_id' => (
    ...
    element_attr => {
        rel => ['tooltip'],
        title => ['The billing profile id used to charge this contract. It becomes active immediately.']
    },
);
...
has_field 'billing_profiles' => (
    ...
    element_attr => {
        rel => ['tooltip'],
        title => ['As an alternative to a single billing profile id, the billing profile / billing network interval schedule used to charge this contract can be specified. It is represented by an array of objects, each containing the keys "start", "stop", "profile_id" and "network_id" (/api/customers/ only). When POSTing, it has to contain a single interval with empty "start" and "stop" fields. Only intervals beginning in the future can be updated afterwards.']
    },
);
