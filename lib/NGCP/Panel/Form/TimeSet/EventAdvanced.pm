...
has_field 'repeat.freq' => (
    ...
    options => [
        ...
        map { +{value => $_, label => ucfirst($_)}; } (qw/second minute hour day week month year/)
    ],
    element_attr => {
        rel => ['tooltip'],
...
has_field 'byday.weekdays' => (
    ...
    label => 'By week day',
    options => [
        map { +{value => substr($_,0,2), label => substr($_,0,2)}; } (qw/SUN MON TUE WED THU FRI SAT/)
    ],
    label_attr => {
        rel => ['tooltip'],
...
has_field 'bymonth' => (
    ...
    label => 'By month',
    options => [
        map { +{value => substr($_,0,3), label => substr($_,0,3) }; } (qw/January February March April May June July August September October November December/)
    ],
    min_value => 1,
    max_value => 12,
    label_attr => {
...
has_block 'fields' => (
    ...
    class => [qw/modal-body/],
    #duration
    render_list => [qw/id comment start end repeat repeat_stop byhour byday bymonth bymonthday bysetpos byweekno byyearday bysecond byminute/],
);
...
sub switch_end_control {
    ...
    my $form = $self->form;
    my $ctx = $form->ctx;
    my $element = NGCP::Panel::Block::Block->new( c => $ctx, template => 'timeset/switch_end_control.tt', form => $form);
...
sub wrap_label_field_switch {
    ...
    my $form = $self->form;
    my $ctx = $form->ctx;
    my $element = NGCP::Panel::Block::Block->new( c => $ctx, template => 'timeset/switch_field_label.tt', form => $form);
    return $element->render({ field => $self });
}
...
sub custom_get_values {
    ...
    foreach my $join_field (@join_fields) {
        $values->{$join_field} = join(',',@{$fif->{$join_field}});
    }
    if ($fif->{end} && $fif->{end}->{switch}) {
        $values->{dtend} = join('T', @{$fif->{end}}{qw/date time/});
    }
    if ($fif->{byday}) {
