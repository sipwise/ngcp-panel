...
use JSON::Types;
use NGCP::Panel::Form::CFSimpleDNDAPI;
use NGCP::Panel::Utils::Subscriber;
use NGCP::Panel::Utils::Preferences;
use Readonly;
use Data::Dumper;

Readonly my $dset_name => 'dnd_autoset';

sub get_form {
    my ($self, $c) = @_;

    return NGCP::Panel::Form::CFSimpleDNDAPI->new(ctx => $c);
}
...
sub _get_content {
    ...
    return { subscriber_id => $prov_sub->id,
...
sub _item_rs {
    ...
    if ($c->request->method eq 'GET') {
        $filter = { 'destination_set.name' => 'dnd_autoset' };
    }
