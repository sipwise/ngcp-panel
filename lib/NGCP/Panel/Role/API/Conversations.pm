...
sub _item_rs {
    ...
    if ($params->{subscriber_id}) {
        ...
        $uuid = $subscriber->uuid if $subscriber;
    }
    if ($params->{customer_id}) {
        # ensure integer, allow terminated
        my $contratct;
        eval {
            $contratct = $c->model('DB')->resultset('contract')->find($params->{subscriber_id});
        };
        $contract_id = $contratct->id if $contratct;
    }
    ...
    foreach my $type (qw(call voicemail sms fax xmpp)) {
        $show->{$type} = ((defined $type_param) ? (index(lc($type_param),$type) > -1) : 1);
...
sub _apply_timestamp_from_to {
    my ($self,$rs,$params,$col,$cond_code);
    $cond_code //= sub { return shift->epoch; };
    if (exists $params->{from} && (my $from = NGCP::Panel::Utils::DateTime::from_string($params->{from}))) {
        $rs = $rs->search_rs(
            { $col => { '>=' => &$cond_code($from) } };
        );
    }
    if (exists $params->{to} && (my $to = NGCP::Panel::Utils::DateTime::from_string($params->{to}))) {
        $rs = $rs->search_rs(
            { $col => { '<=' => &$cond_code($to) } };
        );
    }
...
sub _get_call_rs {
    ...
    $rs = $self->_apply_timestamp_from_to($rs,$params,'me.start_time');
    if ($provider_id) {
        $rs = $rs->search({
            -or => [
...
sub _get_voicemail_rs {
    ...
    $rs = $self->_apply_timestamp_from_to($rs,$params,'me.origtime');
    if ($reseller_id) {
        $rs = $rs->search({
...
sub _get_sms_rs {
    ...
    $rs = $self->_apply_timestamp_from_to($rs,$params,'me.time');
    if ($reseller_id) {
        $rs = $rs->search_rs({
            'contact.reseller_id' => $reseller_id,
        },{
...
sub _get_xmpp_rs {
    ...
    $rs = $self->_apply_timestamp_from_to($rs,$params,'me.epoch');
    if ($reseller_id) {
        $rs = $rs->search_rs({
            'contact.reseller_id' => $reseller_id,
        },{
