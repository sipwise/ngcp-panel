...
sub check_put2get{
    ...
    if(!$check_cb_or_switch || 'CODE' eq ref $check_cb_or_switch){
        is_deeply($put_out->{content_in}, $get_out->{content}, "$self->{name}: check_put2get: check PUTed item against GETed item");
    }
...
sub http_code_msg{
    ...
    if ( ($res->code < 300) || ( $code >= 300 ) ) {
        $message_res = "$self->{name}: ".$message;
    } else {
        $content //= $self->get_response_content($res);
        if (defined $content && $content && defined $content->{message}) {
            $message_res = "$self->{name}: ".$message . ' (' . $res->message . ': ' . $content->{message} . ')';
        } else {
            $message_res = "$self->{name}: ".$message . ' (' . $res->message . ')';
        }
    }
