...
foreach my $api (@apis){
   ...
   (undef, $preferences_put->{content}) = $test_machine->request_put($preferences_old,$preferences->{uri});
    ...
    if ($api eq 'subscriber') {
        ...
        my $preferences_new;
        my $addmulti_value = ['111','222','333'];
        my $preferences_patch_op = [
            {'op' => 'addmulti', 'path' => '/allowed_clis/-', value => ['111','222','333'] },
        ];
        ($res, $preferences_put->{content}) = $test_machine->request_patch($preferences_patch_op,$preferences->{uri});
        $test_machine->http_code_msg(200, "check extended patch result", $res, $preferences_put->{content});
        (undef, $preferences_new->{content}) = $test_machine->check_item_get($preferences->{uri});
        is_deeply([@{$preferences_new->{content}->{allowed_clis}}[-3..-1]], $addmulti_value, "check patched allowed_clis");

        $preferences_patch_op = [
            {'op' => 'remove', 'path' => '/allowed_clis/a1', value => '222' },
        ];
        ($res, $preferences_put->{content}) = $test_machine->request_patch($preferences_patch_op,$preferences->{uri});
        $test_machine->http_code_msg(200, "check extended patch result", $res, $preferences_put->{content});
        (undef, $preferences_new->{content}) = $test_machine->check_item_get($preferences->{uri});
        is_deeply([@{$preferences_new->{content}->{allowed_clis}}[-2..-1]], ['111','333'], "check patched allowed_clis");
    }
