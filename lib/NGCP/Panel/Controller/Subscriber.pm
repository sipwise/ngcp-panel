...
sub calllist_master :Chained('base') :PathPart('calls') :CaptureArgs(0) {
    ...
    push @{ $call_cols }, (
        { name => "source_customer_cost", search => 1, title => $c->loc('Cost'), show_total => 'sum',
            literal_sql => 'if(source_user_id = "'.$c->stash->{subscriber}->uuid.'", source_customer_cost, destination_customer_cost)' },
    ) ;
