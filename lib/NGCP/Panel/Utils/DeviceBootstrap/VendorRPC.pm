...
has '_ua' => (
    ...
    default => sub {
        ...
        if ($c && ref $cfg eq 'HASH' && $cfg->{vendor}) {
            ...
            if ($autoprov_config->{vendors_ssl_verify}) {
                if (grep {$_ eq $vendor} split(/\W+/, $autoprov_config->{vendors_ssl_verify})){
                    $verify_ssl = 0;
                }
                if (grep {$_ eq $vendor} split(/\W+/, $autoprov_config->{vendors_ssl_verify_hostname})){
