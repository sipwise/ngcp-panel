...
sub insert {
    ...
    save_e164(event => $event, number => $subscriber->primary_number, types_prefix => 'primary_number_', now_hires => $now_hires);
    ...
    if ($bm_actual->billing_mappings->first->product->product_class eq 'pbxaccount') {
        ...
        if ($pilot) {
            save_e164(event => $event, number => $pilot->primary_number, types_prefix => 'pilot_primary_number_', now_hires => $now_hires);
        }
    }
}
...
sub save_e164 {
    my %params = @_;
    my ($event,$number,$types_prefix,$now_hires) = @params{qw/event number types_prefix now_hires/};
    if ($number) {
        my $tags_rs = $schema->resultset('events_tag');
        my $relations_rs = $schema->resultset('events_relation');
        $event->create_related("relation_data", {
            relation_id => $relation_rs->find({ type => $types_prefix.'_id' })->id,
            val => $number->id,
