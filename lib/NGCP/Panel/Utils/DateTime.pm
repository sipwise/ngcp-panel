...
sub sec_to_hms {
    ...
    $secs_digits //= 0;
    my ($string,$years,$months,$days,$hours,$minutes,$seconds) = get_duration_string($c,$secs,'hours','seconds',$sec_decimals);
    my $result = sprintf("%d", $hours) . ':' . sprintf("%02d", $minutes) . ':' . sprintf("%02d", $seconds);
    my $fractional_secs;
    if ($sec_decimals > 0 && ($fractional_secs = $seconds - int($seconds)) > 0.0) {
        $result .= '.' . substr(sprintf('%.' . $sec_decimals . 'f', $sec_decimals),2);
    }
    return $result;
}
...
sub get_duration_string {
	my ($c,$duration,$most_significant,$least_significant,$least_significant_decimals) = @_;
	my $abs = abs($duration);
