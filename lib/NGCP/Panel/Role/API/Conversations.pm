...
sub get_form {
    my ($self, $c) = @_;
    return (NGCP::Panel::Form::get("NGCP::Panel::Form::Conversation::API", $c),['id','call_id']);
}
...
sub process_hal_resource {
    ...
    my $item_mock_obj = _get_item_object($c, $item);
...
sub _get_fields_names{
    ...
    return { map { $_ => $call_fields_tied->Indices($_) ? _get_alias($call_fields_tied->Indices($_) + 1) : $_; } (keys %call_fields, 'id') };
}
