    ...
    if($prov_subscriber->profile_id && (
       $c->user->roles eq "subscriberadmin" || $c->user->roles eq "subscriber")) {
        ...
        foreach my $pref(qw/cfu cft cfna cfb cfs cfr cfo
                            ...
                            voice_mail fax_server phonebook
                            csc_registered_devices csc_conversations
                            csc_device_provisioning csc_hunt_groups/) {
...
sub edit_master :Chained('master') :PathPart('edit') :Args(0) :Does(ACL) :ACLDetachTo('/denied_page') :AllowedRole(admin) :AllowedRole(reseller) :AllowedRole(ccareadmin) :AllowedRole(ccare) :AllowedRole(subscriberadmin) {
    ...
    my $is_pbx_customer = $subscriber->contract->product->class eq "pbxaccount";
    my $base_number;
