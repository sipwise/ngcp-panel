...
SKIP:
{
    ...
    my $destinationset = _create_cfdestinationset($subscriber,[{ destination => "1234",
        timeout => '10',
        priority => '1',
    ...
    my $timeset = _create_cftimeset($subscriber,[{ year => $year + 1900,
                  ...
                  hour => $hour,
                  minute => $min}]);
    my $mapping = _create_cfmapping($subscriber,{
        ...
        cfna => [],
        cft => [],
        cfu => [{ destinationset => $destinationset->{name},
