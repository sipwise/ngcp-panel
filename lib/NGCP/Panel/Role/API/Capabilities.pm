...
sub _item_rs {
    ...
    if($c->user->roles eq "admin") {
        ...
        $rtcengine = $rtcengine & $c->user->reseller->rtc_user;
    } else {

        my $customer = NGCP::Panel::Utils::Contract::get_customer_rs(
            c => $c,
            contract_id => $c->user->account_id,
        )->first;

        #$cloudpbx = $cloudpbx & ($customer->get_column('product_class') eq 'pbxaccount'));

        # TODO: fetch subscriber profile if any, and restrict access from there!
        # relevant for sms (?), rtcengine (?), faxserver (?)
        #my $profile = $c->user->voip_subscriber_profile;
        #if($profile) {
        #   $attr = $profile->profile_attributes;

           #my $faxattr = $attr->attribute->find({ attribute => 'fax_server' });
           #if(!$faxattr || $faxattr->value
