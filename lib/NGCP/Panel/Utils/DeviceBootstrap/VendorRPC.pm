...
has '_ua' => (
    ...
    default => sub {
        ...
        if ($c && ref $cfg eq 'HASH' && $cfg->{vendor}) {
            my $vendor = lc($cfg->{vendor} // '');
            my $autoprov_config = lc($c->config->{deviceprovisioning});
            if ($autoprov_config->{vendors_skip_ssl_verify}) {
                if (grep {$_ eq $vendor} split(/\W+/, $autoprov_config->{vendors_skip_ssl_verify})){
                    $verify_ssl = 0;
                }
            }
            if ($autoprov_config->{vendors_skip_ssl_verify_hostname}) {
                if (grep {$_ eq $vendor} split(/\W+/, $autoprov_config->{vendors_skip_ssl_verify_hostname})){
