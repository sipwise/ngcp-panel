...
sub get_pbx_subscribers_rs{
    ...
    return $schema->resultset('voip_subscribers')->search_rs(
        {
            status => { '!=' => 'terminated' },
            $ids ? ( id => { -in => \@$ids } ) : (),
...
sub get_pbx_subscribers_by_ids{
    ...
    if($#items < $#{$params{ids}}){
        my %items_ids_exists = map{ $_->id => 1 } @items;
...
sub manage_pbx_groups{
    ...
    my ($groups)  = $params{groups} // get_pbx_subscribers_by_ids( 
        c           => $c, 
        schema      => $schema, 
        ids         => $group_ids, 
    ...
    my ($groupmembers) = $params{groupmembers} // get_pbx_subscribers_by_ids( 
        c           => $c, 
        schema      => $schema, 
        ids         => $groupmember_ids, 
        customer_id => $customer->id, 
    ...
    $prov_subscriber->voip_pbx_groups->delete_all();
