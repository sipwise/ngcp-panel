...
sub get_parent_info {
    ...
    if (!$tz_owner_parent_type) {
        if ($c->stash->{close_target}) {
            my $close_target = $c->stash->{close_target};
            if ($close_target =~m!/(contract|customer)(?:/[^0-9]+)?/([0-9]+)/!) {
                $tz_owner_parent_type = 'reseller';
                #billing subscriber
                $tz_owner_parent_id = $schema->resultset('contracts')->search_rs({contract_id => $2})->first->contact->reseller_id;
                if (!$tz_owner_parent_id ) {
                    $tz_owner_parent_type = '';
                }
                ...
                $tz_owner_parent_type = 'contract';
                #billing subscriber
                $tz_owner_parent_id = $schema->resultset('voip_subscribers')->search_rs({contract_id => $2})->first->contact->reseller_id;
                if (!$tz_owner_parent_id ) {
                    $tz_owner_parent_type = '';
                }
            }
