...
my $apis_config = {
    subscriber => {
        put_processing => sub {delete $_[1]->{content}->{subscriber_id};},
    },
    domain => {
        put_processing => sub {delete $_[1]->{content}->{domain_id};},
    },
    peeringserver => {
        put_processing => sub {delete $_[1]->{content}->{peering_id};},
    },
    customer => {
        put_processing => sub {delete $_[1]->{content}->{customer_id};},
    },
    profile => {
        put_processing => sub {delete $_[1]->{content}->{profile_id};},
    },
};
...
foreach my $api (@apis){
    ...
    (undef, $preferences_put->{content}) = $test_machine->check_put2get({data_in=>$preferences->{content},uri=>$preferences->{uri}},undef, $apis_config->{$api}->{put_processing} // 1);
