...
sub check_profile_set_and_profile {
    ...
    if($c->user->roles eq "admin") {
        ...
        if ($c->stash->{pilot} && $c->stash->{pilot}->provisioning_voip_subscriber->profile_set_id) {
            ...
            #although subscriberadmin is not supposed to add a pilot,
    ...
    if ($resource->{profile_set}{id}) {
        ...
        #we ignore here checking of belongings to logged in subscriberadmin
    ...
    $c->log->debug("profile_set:".($profile_set ? "true":"false").";profile:".($profile ? "true":"false").";");
    if($profile_set && !$profile) {
        $c->log->debug("We will select now default profile;");
        $profile = $profile_set->voip_subscriber_profiles->find({
            set_default => 1,
        });
    }
    if ($resource->{profile}{id} && !$profile) {
        return {
