...
sub _process_calls_rows {
    ...
    NGCP::Panel::Utils::Datatables::process(
        ...
        sub {
            ...
            $data{source_customer_cost} = (defined $result->get_column('source_customer_cost') ? sprintf("%.2f", $result->get_column('source_customer_cost') / 100.0) : "");
            return %data;
        },
        sub {
            my ($result) = @_;
            my %data = ();
            $data{duration} = (defined $result->{duration} ? sprintf("%.2f s", $result->{duration}) : "");
            $data{source_customer_cost} = (defined $result->{source_customer_cost} ? sprintf("%.2f", $result->{source_customer_cost} / 100.0) : "");
            return %data;
        },
    );
}

sub ajax_calls :Chained('master') :PathPart('list/ajax') :Args(0) {
    my ($self, $c) = @_;
...
sub ajax_calls_in :Chained('master') :PathPart('list/ajax/in') :Args(0) {
    my ($self, $c) = @_;
...
sub ajax_calls_out :Chained('master') :PathPart('list/ajax/out') :Args(0) {
    my ($self, $c) = @_;
