...
sub update_customer {
    ...
    #my $billing_mapping = $customer->billing_mappings->find($customer->get_column('bmid'));
    my $billing_profile = $customer->billing_mappings->find($customer->get_column('bmid'))->billing_profile;
    #$old_resource->{billing_profile_id} = $billing_profile->id;
    ...
    try {
        ...
        NGCP::Panel::Utils::Contract::remove_future_billing_mappings($customer) if $is_multi_bm_mode;
        foreach my $mapping (@$mappings_to_create) {
            $customer->billing_mappings->create($mapping); 
        }
        $customer = $self->customer_by_id($c, $customer->id);
        $billing_profile = $customer->billing_mappings->find($customer->get_column('bmid'))->billing_profile;
