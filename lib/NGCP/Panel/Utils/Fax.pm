...
sub process_fax_journal_item {
    ...
    if ($dir eq 'out') {
        ...
        if ($prov_sub->pbx_extension) {
            ...
            # otherwise its a regular alias/no pbx, no special override.
        }
        ...
        if ($src_sub && $dst_sub && $src_sub->contract_id == $dst_sub->contract_id) {
            ...
            if ($prov_dst_sub->pbx_extension) {
                ...
                # otherwise its a non-pbx intra customer fax, no special override.
            }
        } else {
            # fax to other local subscriber, maybe other pbx, no override.
        }
        ...
        if ($src_sub && $dst_sub && $src_sub->contract_id == $dst_sub->contract_id) {
            ...
            # fax from other local subscriber, maybe other pbx, no override.
        }
    }
