...
sub devices_preferences_create :Chained('devices_preferences_list') :PathPart('create') :Args(0) {
    ...
    if($posted && $form->validated) {
        try {
            my $schema = $c->model('DB');
            $schema->txn_do(sub {
                my $preference = NGCP::Panel::Utils::Preferences::create_dev_dynamic_preference(
                    $c, $form->values, devmod => $c->stash->{devmod} 
                );
...
sub devices_preferences_editmeta :Chained('devices_preferences_base') :PathPart('editmeta') :Args(0) {
    ...
    if($posted && $form->validated) {
        try {
            my $schema = $c->model('DB');
            $schema->txn_do(sub {
                NGCP::Panel::Utils::Preferences::update_dev_dynamic_preference($c, $c->stash->{preference_meta}, $form->values);
             });
            NGCP::Panel::Utils::Message::info(
                c    => $c,
