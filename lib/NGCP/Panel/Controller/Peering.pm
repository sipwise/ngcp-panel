...
use NGCP::Panel::Form::Peering::RuleEdit;
use NGCP::Panel::Form::Peering::InboundRule;
use NGCP::Panel::Form::Peering::InboundRuleEdit;
...
sub rules_edit :Chained('rules_base') :PathPart('edit') :Args(0) {
    ...
    my $posted = ($c->request->method eq 'POST');
    my $form = NGCP::Panel::Form::Peering::RuleEdit->new(ctx => $c);
...
sub inbound_rules_edit :Chained('inbound_rules_base') :PathPart('edit') :Args(0) {
    ...
    my $posted = ($c->request->method eq 'POST');
    my $form = NGCP::Panel::Form::Peering::InboundRuleEdit->new(ctx => $c);
    $c->stash->{rule}{group}{id} = delete $c->stash->{rule}{group_id};
    $form->process(
        posted => $posted,
        params => $c->request->params,
