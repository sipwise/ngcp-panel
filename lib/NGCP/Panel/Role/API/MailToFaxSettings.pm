...
sub hal_from_item {
    ...
    for my $notify ($prov_subs->voip_mail_to_fax_secret_renew_notify->all) {
        push @secret_renew_notify, {$notify->get_inflated_columns};
    }
    $resource{secret_renew_notify} = \@secret_renew_notify;
    my @acls;
    for my $acl ($prov_subs->voip_mail_to_fax_acl->all) {
...
sub update_item {
    ...
    my $secret_renew_notify_rs = $prov_subs->voip_mail_to_fax_secret_renew_notify;
    my $acl_rs = $prov_subs->voip_mail_to_fax_acl;
