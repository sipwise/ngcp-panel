...
sub prepare_peaktime_weekdays {
    ...
    return 1 unless 'ARRAY' eq ref $resource;
    ...
    my %intersecter_map = ();
    foreach my $peaktime_weekday (@$resource) {
        ...
        $stime = $parsetime->format_datetime($stime);
        $etime = $parsetime->format_datetime($etime);
        if ($end < $start) {
            return 0 unless &{$err_code}("Peaktime $WEEKDAYS{$weekday} end time $etime must be later than start time $stime.");
        }
        ...
        if (defined $intersecter) {
            ...
            my $label = $WEEKDAYS{$weekday} . ' (' . $stime . ' - ' . $etime .' )';
...
sub prepare_peaktime_specials {
    ...
    foreach my $peaktime_special (@$resource) {
...
sub resource_from_peaktime_weekdays {
    ...
    foreach my $weekday_peaktime ($rs->all) {
        ...
        $wp{start} = ($weekday_peaktime->start ? $datetime_fmt->format_datetime($weekday_peaktime->start) : undef);
        $wp{stop} = ($weekday_peaktime->end ? $datetime_fmt->format_datetime($weekday_peaktime->end) : undef);
