...
sub insert {
    ...
    save_e164(event => $event, number => $subscriber->primary_number, types_prefix => 'number_', now_hires => $now_hires);
    ...
    if ($bm_actual->billing_mappings->first->product->product_class eq 'pbxaccount') {
        ...
        if ($pilot) {
            save_e164(event => $event, number => $pilot->primary_number, types_prefix => 'pilot_number_', now_hires => $now_hires);
        }
    }
}
