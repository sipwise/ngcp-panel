...
sub create_dynamic_preference {
    ...
    $resource->{voip_preference_groups_id} = $c->model('DB')->resultset('voip_preference_groups')->find({name => })->id;
    ...
    if (defined $resource->{dev_pref} && $resource->{dev_pref} && $devmod) {
...
sub update_dynamic_preference {
    ...
    $preference->voip_preference_relations->delete;
    if (defined $resource->{dev_pref} && $resource->{dev_pref} && $devmod) {
        $preference->create_related('voip_preference_relations', {
            autoprov_device_id => $devmod->id
        });
    ...
    if(defined $enums and ref $enums eq 'ARRAY'){
        ...
        my @old_enum_ids =  $old_enums_rs->id->all;
        my %old_enum_ids;
        ...
        $enums_rs->search_rs({ id => { -in => [keys %old_enum_ids ] } )
    } else {
