...
sub receive :Chained('list') :PathPart('receive') :Args(0) {
    ...
    try {
        ...
        $schema->txn_do(sub {
            ...
            my $smslog = "SMS from $from to $to (prov sub id ".$prov_dbalias->subscriber_id."):";
            ...
            unless($cf_maps->count) {
                $c->log->info("$smslog No cfs for inbound sms from $from to $to found.");
                last;
            }
            ...
            foreach my $map($cf_maps->all) {
                ...
                if($map->source_set) {
                    ...
                    foreach my $source($map->source_set->voip_cf_sources->all) {
                        $c->log->debug("$smslog checking $from against ".$source->source);
                        if(fnmatch($source->source, $from)) {
                            $c->log->debug("$smslog matched $from against ".$source->source.", pass");
                            $source_pass = 1;
                            last;
                        }
                    }
                ...
                if($source_pass) {
                    $c->log->debug("$smslog source check for $from passed, continue with time check");
                } else {
                    $c->log->debug("$smslog source check for $from failed, trying next map entry");
                    next;
                }
                ...
                if($map->time_set) {
                    ...
                    foreach my $time($map->time_set->voip_cf_periods->all) {
                        ...
                        $c->log->debug("$smslog checking $now against ".$timestring);
                        if(inPeriod($now, $timestring)) {
                            $c->log->debug("$smslog matched $now against ".$timestring.", pass");
                            $time_pass = 1;
                            last;
                        }
                    }
                ...
                if($time_pass) {
                    $c->log->debug("$smslog time check for $now passed, use destination set");
                    $dset = $map->destination_set;
                    last;
                } else {
                    $c->log->debug("$smslog time check for $now failed, trying next map entry");
                    next;
                }
            }

            unless($dset) {
                $c->log->info("$smslog no CF checks matched, stop without forwarding");
                last;
            }

            $c->log->debug("$smslog proceed with sms forwarding");

            unless($dset->voip_cf_destinations->first) {
                $c->log->info("$smslog the detected cf destination set id " . $dset->id ." has no destinations, stop without forwarding");
                last;
            }
            my $dst = $dset->voip_cf_destinations->first->destination;
            $dst =~ s/^sip:(.+)\@.+$/$1/;
            $c->log->debug("$smslog forward sms to $dst");

            # TODO: send 3rd party api request and wait for ok before forwarding?

            # TODO: use user_cli or even original from for new caller?
