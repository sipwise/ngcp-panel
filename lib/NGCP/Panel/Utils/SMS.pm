...
sub perform_prepaid_billing {
    ...
    die "Failed to create sms amqr handle" unless $amqr;
    ...
    for(my $i = 0; $i < $parts; ++$i) {
        ...
        die "Failed to create sms rating session" unless($sess);
        # TODO: if session is blocked, $sess is 0 anyways, so the
        # line below would never execute
        die "SMS rating session is blocked" if(NGCP::Rating::Inew::SmsSession::session_blocked($sess));
        if(NGCP::Rating::Inew::SmsSession::session_sms_reserve($sess)) {
            push @sessions, $sess;
        } else {
            last;
        }
    ...
    if(@sessions == $parts) {
        ...
        die "Failed to reserve credit for all sms parts";
    }
