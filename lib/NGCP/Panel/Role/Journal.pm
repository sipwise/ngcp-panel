...
sub get_journal_operation_config {
    ...
    return $c->stash->{journal_operation_config}, $c->stash->{journal_operation}
        if $c->stash->{journal_operation_config} && $c->stash->{journal_operation} && !$reload;
    my $operation;
    if (!$operation_spec) {
    ...
    if ($operation_spec eq 'create') {
        ...
        $operation = NGCP::Panel::Utils::Journal::DELETETE_JOURNAL_OP;
    }
    my $cfg = _get_api_journal_op_config($c, $self->resource_name, $operation);
