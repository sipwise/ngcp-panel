...
sub process {
    ...
    ($displayRecords, $displayRecordCountClipped, my $is_set_operations) = _get_count_safe($c,$rs,$params) if(!$use_rs_cb);
    ...
    if (!$use_rs_cb) {
        if (@searchColumns) {
            ...
            #if (!@$aggregate_cols) {
            #    ($displayRecords, $displayRecordCountClipped) = _get_count_safe($c,$rs,$params);
            #}
        } else {
