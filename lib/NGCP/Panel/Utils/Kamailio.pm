...
#See detailed description of the implemented logic in the workfron tasks 37459, 14589 
    ...
    my $subscriber_lb_ptr;
    if ($subscriber_lb_ptr_preference_rs && $subscriber_lb_ptr_preference_rs->first) {
        $c->log->debug('lbrtp_set:'.$subscriber_lb_ptr_preference_rs->first->value);
    ...
    if (!$subscriber_lb_ptr) {
        ...
        $subscriber_lb_ptr = $lb_clusters->{$lb_clusters->{default}} // 'sip:lb@127.0.0.1:5060;lr';
    }
