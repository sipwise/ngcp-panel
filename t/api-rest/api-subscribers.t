...
if($remote_config->{config}->{features}->{cloudpbx}){
    ...
    #TT#50802
        ...
        my $terminated_primary_number = $test_machine->get_collection_hal('numbers', '/api/numbers/?sn=12508021', 1);
        diag("50802: primary_number of member_to_terminate");
        #print Dumper $terminated_primary_number;
        
        diag("50802: terminate member_to_terminate");
        $test_machine->request_delete($member_to_terminate->{location});

        $terminated_primary_number = $test_machine->get_collection_hal('numbers', '/api/numbers/?sn=12508021', 1);
        diag("50802: primary_number of member_to_terminate after termination:");
        diag(Dumper($terminated_primary_number));

        diag("50802: recreate pbx extension after termination:");
        my $member_to_terminate_next = $test_machine->check_create_correct( 1, sub{
            my $num = $_[1]->{i}.Test::FakeData::seq;
            $_[0]->{username} .= time().'_50802_'.$num ;
            $_[0]->{webusername} .= time().'_'.$num;
            $_[0]->{pbx_extension} = '50802'.'1';
        ...
        #print Dumper $member_to_terminate_next;
        #use Data::Compare;
        foreach (qw/create_timestamp username webusername modify_timestamp id _links uuid/) {
            delete $member_to_terminate->{content}->{$_};
            delete $member_to_terminate_next->{content}->{$_};
        }
        is_deeply($member_to_terminate->{content}, $member_to_terminate_next->{content},"check members to terminate");
        ...
        #my $pilot_numbers = $test_machine->get_collection_hal('numbers', '/api/numbers/?subscriber_id='.$pilot_local->{content}->{id}, 1)->{collection};
        #print Dumper $pilot_numbers;
        ...
        #my $terminated_primary_number = $test_machine->get_collection_hal('numbers', '/api/numbers/?type=alias&sn=12508021', 1);
        #print Dumper $terminated_primary_number;
