...
at your option, any later version of Perl 5 you may have available.
    ...
    my $name = 'pbxdevices';
    while (my @keys_and_value = reach($data->{$name}->{data})){
        print Dumper [@keys_and_value];
        my $field_value = pop @keys_and_value;
        print Dumper [$field_value, ('CODE' eq ref $field_value)];
        if('CODE' eq ref $field_value ){
            my $value = 'xxx';
            print "value=$value;\n";
            #print Dumper [$data->{$name}->{data}, @keys_and_value, $value];
            nest( $data->{$name}->{data}, @keys_and_value, $value );
        }
        #sleep 1;
    }
    print Dumper $data->{$name}->{data};
    die();
