...
sub master :Chained('base') :PathPart('details') :CaptureArgs(0) {
    ...
    push @{ $call_cols }, (
        { name => "total_customer_cost", search => 1, title => $c->loc('Customer Cost (cents)'), show_total => 'sum', literal_sql => 'if(source_user_id = "'.$c->stash->{subscriber}->uuid.'", source_customer_cost, destination_customer_cost)'.$vat_factor },
    ) ;
