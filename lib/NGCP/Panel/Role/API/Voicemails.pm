...
sub get_form {
    my ($self, $c) = @_;
    require NGCP::Panel::Form::Voicemail::Meta;
...
sub hal_from_item {
    ...
    my $hal = NGCP::Panel::Utils::DataHal->new(
        links => [
            ...
            NGCP::Panel::Utils::DataHalLink->new(relation => 'ngcp:subscribers', href => sprintf("/api/subscribers/%d", $item->mailboxuser->provisioning_voip_subscriber->voip_subscriber->id)),
            NGCP::Panel::Utils::DataHalLink->new(relation => 'ngcp:voicemailrecordings', href => sprintf("/api/voicemailrecordings/%d", $item->id)),
        ],
