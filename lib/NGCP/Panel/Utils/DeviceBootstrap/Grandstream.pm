...
sub register_content {
    ...
    $self->{register_content} = 
        '{"cid":"'.($self->params->{cid} // '2018')
        .'","method":"redirectDefault","params":{"macs":["'
        .$self->content_params->{mac}.'"]}}';
    #test request
    $self->{register_content} = '{"cid":"'.($self->params->{cid} // '2018').'","method":"listProfile","params":{}}';
...
override 'parse_rpc_response_page' => sub {
    ...
    #irka::loglong(Dumper(["register_unregister response",$res]));
    return $res;
};
...
sub get_server_time {
    ...
    my $req = HTTP::Request->new('GET', $self->rpc_server_params->{proto}.'://'.$self->rpc_server_params->{host}.$self->rpc_server_params->{path});
    my $time_response = $ua->request($req);
    #irka::loglong(Dumper(["time_response",$time_response]));
...
sub get_request_sign{
    ...
    #run3 \@cmd, \$in, \$out, \$err;
    #here is example without using input:
    #http://unix.stackexchange.com/questions/149169/how-to-pass-string-not-file-to-openssl
    #sig=$(openssl sha1 -hmac $key -binary <(echo -n $str)|xxd -p)
    #so we may try to avoid Run3 with usual capturex
    run3 $cmd, \$str2sign, \$sign, \$sign_error;
    $sign=~s/\n//g;
    #irka::loglong(Dumper(["sign",$sign]));
    return ($sign, $time);
}
1;
