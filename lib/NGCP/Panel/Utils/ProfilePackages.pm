...
use constant _CARRY_OVER => 'carry_over';
...
sub get_contract_balance {
    my %params = @_;
    my($c,$contract,$now,$schema) = qw/c contract now schema/;
    
    $schema //= $c->model('DB');
    $now //= NGCP::Panel::Utils::DateTime::current_local;
    
    return catchup_contract_balances(schema => $schema, contract => $contract, now => $now);

}

sub resize_actual_contract_balance {
    my($c,$contract,$old_package,$actual_balance,$now,$schema) = qw/c contract old_package balance now schema/;
    ...
    $now //= $contract->modified_timestamp;
    my $new_package = $contract->profile_package;
    my ($old_start_mode,$new_start_mode);
    
    if (defined $old_package && !defined $new_package) {
        $old_start_mode = $old_package->start_mode if $old_package->start_mode ne _DEFAULT_START_MODE;
        $new_start_mode = _DEFAULT_START_MODE;
    } elsif (!defined $old_package && defined $new_package) {
        $old_start_mode = _DEFAULT_START_MODE;
        $new_start_mode = $new_package->start_mode if $new_package->start_mode ne _DEFAULT_START_MODE;
    } elsif (defined $old_package && defined $new_package && $old_package->start_mode ne $new_package->start_mode) {
        $old_start_mode = $old_package->start_mode;
        $new_start_mode = $new_package->start_mode;
    }
...
sub catchup_contract_balances {
    my %params = @_;
    my($c,$contract,$old_package,$now,$schema) = qw/c contract old_package now schema/;
    ...
    my ($start_mode,$interval_unit,$interval_value,$has_package);
    ...
    my $last_balance = $contract->contract_balances->search(,{ order_by => { '-desc' => 'end'},})->first;
    my $end_of_today = $now->clone->truncate(to => 'day')->add(days => 1)->subtract(seconds => 1);    
    while ($last_balance && $last_balance->end < $end_of_today) {
        my $start_of_next_interval = $last_balance->end->clone->add(seconds => 1);
        
        my $bm_actual;
        unless ($last_profile) {
            $bm_actual = _get_actual_billing_mapping(schema => $schema, contract => $contract, now => $last_balance->start);
            $last_profile = $bm_actual->billing_mappings->first->profile;
        }
        $bm_actual = _get_actual_billing_mapping(schema => $schema, contract => $contract, now => $start_of_next_interval);
        my $profile = $bm_actual->billing_mappings->first->profile;
...
sub XXtopup_create_contract_balance {
    my %params = @_;
    my($c,$contract,$now,$profile,$schema) = qw/c contract now profile schema/;    
    return initial_create_contract_balance(c => $c,contract => $contract,now => $now,$profile,$schema,is_topup => 1);
}

sub create_initial_contract_balance {
    my %params = @_;
    my($c,$contract,$profile,$now,$schema) = qw/c contract profile now schema/;

    $schema //= $c->model('DB');
    $contract = $schema->resultset('contracts')->find({id => $contract->id},{for => 'update'}); #lock record
    $now //= $contract->create_timestamp;
...
sub _get_resized_balance_values {
    my %params = @_;
    my ($c,$balance,$old_start_mode,$new_start_mode,$etime,$schema) = qw/c balance old_start_mode new_start_mode etime schema/;
    
    $schema //= $c->model('DB');
    my ($cash_balance, $free_time_balance) = ($balance->cash_balance,$balance->free_time_balance);
    
    my $contract_create = $balance->contract->create_timestamp;
    if ($balance->start <= $contract_create && $balance->end >= $contract_create) {
        my $bm = _get_actual_billing_mapping(schema => $schema, contract => $balance->contract, now => $contract_create); #now => $balance->start); #end); !?
        my $profile = $bm->billing_mappings->first->profile;
        my $old_ratio = _get_free_ratio($contract->create_timestamp,$old_start_mode,$last_balance->start,$last_balance->end);
        ...
        my $new_ratio = _get_free_ratio($contract->create_timestamp,$new_start_mode,$last_balance->start,$etime);
...
sub _get_balance_values {
    my %params = @_;
    my($c, $profile, $last_profile, $contract, $last_balance, $stime, $etime, $initial_balance, $now, $start_mode) = @params{qw/c profile last_profile contract last_balance stime etime initial_balance now start_mode/};    
    ...
    my $ratio;
    if ($last_balance) {
        if (_CARRY_OVER eq $carry_over_mode || (_CARRY_OVER_TIMELY eq $carry_over_mode && $last_balance->timely_count > 0)) {
            #if (!defined $last_profile) {
            #    my $bm_last = _get_actual_billing_mapping(schema => $schema, contract => $contract, now => $last_balance->start); #end); !?
            #    $last_profile = $bm_last->billing_mappings->first->profile;
            #}
            my $contract_create = $contract->create_timestamp;
            $ratio = 1.0;
...
sub _get_balance_interval_start_end {
    my (%params) = @_;
    my ($now,$start_mode,$last_etime,$interval_unit,$interval_value) = @params{qw/now start_mode last_etime interval_unit interval_value/};
    ...
    if (defined $stime) { #lets crash in the create statement
        if (_TOPUP_START_MODE ne $start_mode) {
            $etime = _add_interval($stime,$interval_unit,$interval_value)->subtract(seconds => 1);
        } else {
...
sub _get_resized_interval_end {
    my (%params) = @_;
    my ($ctime, $create, $start_mode) = qw(/ctime create_timestamp start_mode/);    
...
sub _add_interval {
    my ($from,$interval_unit,$interval_value) = @_;
    if ('day' eq $interval_unit) {
        ...
        return $from->clone->add(months => $interval_value, end_of_month => 'preserve');
    }
    return undef;
}
...
sub _get_actual_billing_mapping {
    my %params = @_;
    my ($schema,$contract,$now) = $params{qw/schema contract now/};
    $now //= NGCP::Panel::Utils::DateTime::current_local;
