...
sub create_dynamic_preference {
    ...
    my $autoprov_devices_id  = delete $resource->{autoprov_devices_id};
    ...
    if (defined $resource->{dev_pref} && $resource->{dev_pref} && $autoprov_devices_id) {
        $preference->create_related('voip_preference_relations', {
            autoprov_devices_id => $autoprov_devices_id
        });
    }
    return $preference;
}
