...
#SKIP:
{
    ...
    my $cc = "888";
    my $pilot_ac = '3'.(scalar keys %subscriber_map);
    my $sn = $t;
    my $pilot_subscriber = _create_subscriber($customer,
        primary_number => { cc => $cc, ac => $pilot_ac, sn => $sn },
    ...
    _check_event_history("events when creating a pbx pilot subscriber: ",$pilot_subscriber->{id},"start_profile",[
        { subscriber_id => $pilot_subscriber->{id}, type => "start_profile",
          ...
          primary_number_cc => $cc, primary_number_ac => $pilot_ac, primary_number_sn => $sn,
          ...
          pilot_primary_number_cc => $cc, pilot_primary_number_ac => $pilot_ac, pilot_primary_number_sn => $sn,
        },
    ]);
