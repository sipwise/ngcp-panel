        ...
        foreach my $pref(@group_prefs) {
            ...
            if($pref->attribute eq "rewrite_rule_set") {
                ...
                $pref->{rwrs_id} = $pref_values->{rewrite_caller_in_dpid} &&
                    ...
                    $tmp->id
                    : undef;
            } elsif($pref->attribute eq "cdr_export_sourceclidestinationuserin_rewrite_rule_set") {
                my $tmp;
...
sub create_preference_form {
    ...
    } elsif ($c->stash->{preference_meta}->attribute eq "cdr_export_sourceclidestinationuserin_rewrite_rule_set") {
    ...
    if($posted && $form->validated) {
        ...
        if ($attribute eq "allowed_ips") {
            ...
            $c->response->redirect($base_uri);
            return 1;
        } elsif ($attribute eq "cdr_export_sourceclidestinationuserin_rewrite_rule_set") {
            my $selected_rwrs = $c->stash->{rwr_sets_rs}->find(
                $form->field($attribute)->value
            );
...
sub api_preferences_defs{
    ...
    for my $pref($preferences->all) {
        ...
        if ($pref->attribute =~ m/^(cdr_export_sourceclidestinationuserin_rewrite_rule_set|rewrite_rule_set|ncos|adm_ncos|adm_cf_ncos|emergency_mapping_container|sound_set|contract_sound_set|header_rule_set)$/) {
