...
sub insert_prof {
    ...
    my $old_profile = $params{old};
    my $new_profile = $params{new};
    ...
    foreach my $new_alias (@$new_aliases) {
        if (not exixts $old_aliases_map->{$new_alias}) {
            if (not $is_pilot and exixts $old_pilot_aliases_map->{$new_alias}) {
    ...
    foreach my $old_alias (@$old_aliases) {
        if (not exixts $new_aliases_map->{$old_alias}) {
            if (not $is_pilot and exixts $new_pilot_aliases_map->{$old_alias}) {
...
sub insert {
    ...
    my $create_event_per_alias = $params{create_event_per_alias};
