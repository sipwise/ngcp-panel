...
sub overall_status {
    ...
    return { class => "ngcp-red-error", text => $c->loc("Errors") } if ( grep { $_->{type} eq 'error' } @{$ngcp_status->{data}} );
    return { class => "ngcp-orange-warning", text => $c->loc("Warnings") } if ( grep { $_->{type} eq 'warning' } @{$ngcp_status->{data}} );
