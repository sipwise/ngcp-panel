...
sub process_fax_journal_item {
    ...
    #try finding it:
    unless ($src_sub) {
        my $prov_src_sub = $c->model('DB')->resultset('provisioning_voip_subscriber')->search({
            'username' => $result->caller,
        })->find();
        if ($prov_src_sub) {
            $src_sub = $src_sub->voip_subscriber;
        }
    }
    my $dst_sub  = $result->callee_subscriber // undef; #undef, if not local, or due to bug in faxserver
    #try finding it:
    unless ($dst_sub) {
        my $dst_src_sub = $c->model('DB')->resultset('provisioning_voip_subscriber')->search({
            'username' => $result->callee,
        })->find();
        if ($prov_dst_sub) {
            $dst_sub = $dst_sub->voip_subscriber;
        }
    }
