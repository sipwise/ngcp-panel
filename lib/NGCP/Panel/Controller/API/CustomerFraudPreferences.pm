...
sub GET :Allow {
    ...
    #my $guard = $c->model('DB')->txn_scope_guard;
    {
        ...
        for my $customer ($customer_rs->all) {
            my $item = $self->item_by_id($c, $customer->id) ||
                $c->model('DB')->resultset('contract_fraud_preferences')->new_result({ contract_id => $customer->id});
            push @embedded, $self->hal_from_item($c, $item, $form);
            push @links, Data::HAL::Link->new(
                relation => 'ngcp:'.$self->resource_name,
                href     => sprintf('/%s%d', $c->request->path, $item->contract->id),
            );
        }
        push @links,
            Data::HAL::Link->new(
