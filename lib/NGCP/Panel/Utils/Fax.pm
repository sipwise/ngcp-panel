...
sub process_fax_journal_item {
    ...
    my $src_sub  = $result->caller_subscriber // undef; #undef, if not local, or due to bug in faxserver
    #try finding it:
    unless ($src_sub) {
    ...
    my $dst_sub  = $result->callee_subscriber // undef; #undef, if not local, or due to bug in faxserver
    #try finding it:
    unless ($dst_sub) {
    ...
    if ($dir eq 'out') {
        ...
        if ($prov_sub->pbx_extension) {
            ...
            if ($resource->{caller} eq $prov_sub->username) {
                ...
                $resource->{caller} = _get_alias_or_primary_number($subscriber);
                $src_rewrite = 0;
        ...
        if ($src_sub && $dst_sub && $src_sub->contract_id == $dst_sub->contract_id) {
            ...
            if ($prov_dst_sub->pbx_extension) {
                ...
                if ($resource->{callee} eq $prov_dst_sub->username) {
                    ...
                    $resource->{callee} = _get_alias_or_primary_number($dst_sub);
                    $dst_rewrite = 0;
        ...
        if ($prov_sub->pbx_extension) {
            ...
            if ($resource->{callee} eq $prov_sub->username) {
                # use its primary number, if the callee field shows a username.
                $resource->{callee} = _get_alias_or_primary_number($subscriber);
                $dst_rewrite = 0;
        ...
        if ($src_sub && $dst_sub && $src_sub->contract_id == $dst_sub->contract_id) {
            ...
            if ($prov_src_sub->pbx_extension) {
                ...
                if ($resource->{caller} eq $prov_src_sub->username) {
                    # use its primary number, if the caller field shows a username.
                    $resource->{caller} = _get_alias_or_primary_number($src_sub);
                    $src_rewrite = 0;
