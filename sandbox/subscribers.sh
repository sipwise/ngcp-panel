...
function get_sub_id {
    username=$1
    id=`curl -i -X GET -H 'Connection: close' -H 'Content-Type: application/json' -H 'Prefer: return=representation' --insecure --user $login:$password 'https://'$host':'$port'/api/subscribers/?username='$username|jq '.["_embedded"] | .["ngcp:subscribers"] | .[] | .["id"]'`;
    echo -n $id;
}
...
for i in $(seq 1 $amount) ; do 
  ...
  suffix=$(get_suffix $i)
  curl -i -X POST -H 'Connection: close' -H 'Content-Type: application/json' -H 'Prefer: return=representation' --insecure --user $login:$password 'https://'$host':'$port'/api/subscribers/' --data-binary '{ "administrative" : false, "customer_id" : '$customer_id', "domain" : "'$domain'", "primary_number" : { "ac" : "1124", "cc" : "2223", "sn" : "4444'$i'" }, "status" : "active", "username" : "'$nameprefix'_'$suffix'", "password" : "'$nameprefix'_pwd_'$suffix'", "display_name" : "'$nameprefix' '$suffix'"}'
...
#
#suffix=$(get_suffix $i)
#id=$(get_sub_id 'subsub_'$suffix)
#
#curl -i -X PATCH -H 'Connection: close'  -H 'Content-Type: application/json-patch+json' -H 'Prefer: return=representation' --insecure --user $login:$password 'https://'$host':'$port'/api/subscriberpreferences/'$id --data-binary '[{ "op": "add", "path" : "/music_on_hold", "value" : true}]'
#
#done
