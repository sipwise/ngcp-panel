...
#goto SKIP;
{
    ...
    $req = HTTP::Request->new('PUT', $customerfraudpreferences_uri); #$customer->{id});
    $req->header('Content-Type' => 'application/json');
    $req->header('Prefer' => 'return=representation');
    $req->content(JSON::to_json({
        #attach => JSON::true,
    ...
    $req = HTTP::Request->new('GET', $customerfraudpreferences_uri); # . '?page=1&rows=' . (scalar keys %$put_data));
    $res = $ua->request($req);
