...
sub _item_rs {
    ...
    if ($c->config->{redis}->{usrloc}) {
        ...
        if ($c->user->roles eq "admin") {
            $item_rs = NGCP::Panel::Utils::Subscriber::get_subscriber_location_rs($c);
        } elsif ($c->user->roles eq "reseller") {
            $item_rs = NGCP::Panel::Utils::Subscriber::get_subscriber_location_rs($c, {reseller_id => $c->user->reseller_id});
        }
    } else {
        my @joins = ();
