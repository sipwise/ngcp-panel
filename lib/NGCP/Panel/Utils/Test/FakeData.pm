package NGCP::Panel::Utils::Test::FakeData;
...
use File::Basename;
use Data::Dumper;
use NGCP::Panel::Utils::Test::DeepHashUtils qw(reach slurp nest deepvalue);
use Clone qw/clone/;

has 'test_machine' =>(
    is => 'rw',
    isa => 'NGCP::Panel::Utils::Test::Collection',
    default => sub { NGCP::Panel::Utils::Test::Collection->new () },
);
has 'created' => (
    is => 'rw',
    isa => 'HashRef',
...
sub create{
    my($self, $name, $parents_in, $params)  = @_;
    $parents_in //= {};
...
sub process{
    ...
    while (my @keys_and_value = reach($self->data->{$name}->{data})){
        ...
        if('CODE' eq ref $field_value ){
            my $value = $field_value->($self,$parents);
