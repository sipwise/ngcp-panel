...
sub clear_cert {
    my $self = shift;
    lock $tmpfilename;
    return unless $tmpfilename;
    unlink $tmpfilename;
    undef $tmpfilename;
    foreach my $clone (@clones) {
    	delete $clone->{ssl_cert};
    	delete $clone->{ua};
    }
}

# the proper solution is not to reset cloned items but to override LWP's
# SSL_cert_file accessor, to make it return $Test::Collection::tmpfilename
sub cloned {
    my $self = shift;
    lock $tmpfilename;
    my $clone = clone $self; #todo: use/implement a copy constructor...
    #delete $clone->{ssl_cert};
    #delete $clone->{ua};
    push(@clones,$clone);
    return $clone;
