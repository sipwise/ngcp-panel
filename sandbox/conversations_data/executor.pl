...
if( 'call' eq $TYPE || 'voicemail' eq $TYPE ){
    ...
    $thread_uas = threads->create(\&xmpp_uas, $ITEM_SERVER, $PORT, @{$REGS->{callee}}{qw/ip domain username password/}, $FILE );
    $thread_uac = threads->create(\&xmpp_uac, $ITEM_AMOUNT, $ITEM_SERVER, $PORT, @{$REGS->{caller}}{qw/ip domain username password/}, join('@', @{$REGS->{callee}}{qw/username domain/}), $FILE );
}
while ((
...
$thread_uas->kill('KILL')->detach();
$thread_uac->kill('KILL')->detach();
