...
SKIP:
{
    ...
    _delete_lnp_numbers(undef,500); #empty number

    _delete_lnp_numbers($number,404); #not found, all records
    ...
    _delete_lnp_numbers($number,404,$now->clone->add(days=>2)); #not found, actual

    _delete_lnp_numbers($number,undef,$start1->clone->add(hours=>1)); #delete actual

    _check_lnpnumber_history("number $number: ",$number,[
        { carrier_id => $carrier2->{id} },
    ],undef);

    _delete_lnp_numbers($number); #delete all
