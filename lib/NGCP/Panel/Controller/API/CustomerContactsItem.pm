...
sub DELETE :Allow {
    ...
    my $guard = $c->model('DB')->txn_scope_guard;
    {
        ...
        if ($contract_rs->first) { #2. if active contracts -> error
            ...
            if ($contract_rs->first or $subscriber_rs->first) { #1. if terminated contracts or subscriber (with any status)
                try {
                    $contact->update({
                        status => "terminated",
                    ...
                    $c->log->error("Failed to update contact id '".$contact->id."': $e");
