...
my $REGS = {};
my( $TYPE, $ITEM_AMOUNT,$SIP_SERVER,$PORT,$FILE);
( $TYPE, $ITEM_AMOUNT, $SIP_SERVER, $PORT,
...
my $thread_uac = threads->create(\&sip_uac, $ITEM_AMOUNT, $SIP_SERVER, $PORT, @{$REGS->{caller}}{qw/ip domain username password/}, join('@', @{$REGS->{callee}}{qw/username domain/}), $FILE );
