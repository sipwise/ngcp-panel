...
sub dispatch_path {
    return '/api/headerruleconditions/';
}

sub relation {
    return 'http://purl.org/sipwise/ngcp-api/#rel-headerruleconditions';
}
...
sub post_process_hal_resource {
    ...
    my @values = ();
    if ($item->rwr_set) {
        my %set_cols = $item->rwr_set->get_inflated_columns;
        foreach my $dp_t (qw(callee_in callee_out caller_in caller_out)) {
            my $c_dp_id = $set_cols{$dp_t.'_dpid'} // next;
            if ($c_dp_id == $dp_id) {
    ...
    delete $resource->{value_group_id};
    return $resource;
}
...
sub check_resource {
    ...
    unless ($resource->{rwr_dp}) {
        $self->error($c, HTTP_UNPROCESSABLE_ENTITY, "Missing 'rwr_dp' (when rwr_dp_id is set).");
        return;
    }

    return 1 if $resource->{rwr_dp_id};
...
sub update_item_model {
    ...
    my $guard = $schema->txn_scope_guard;
    {
        try {
            ...
            $c->log->error("failed to update header rule condition: $e");
            $self->error($c, HTTP_INTERNAL_SERVER_ERROR, "Failed to update header rule condition.");
            return;
        }
        $guard->commit;
    }
