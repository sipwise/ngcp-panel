...
SKIP:
{
    ...
    _delete_lnp_numbers($number,404);

    my $number1 = _create_lnp_number($carrier1, number => $number);
    my $number2 = _create_lnp_number($carrier2, number => $number);
...
sub _delete_lnp_numbers {
    ...
    my ($number,$expected_code) = @_;
    $expected_code //= 204;
    my $url = $uri.'/api/lnpnumbers/?number='.$number;
    $req = HTTP::Request->new('DELETE', $url);
    $res = $ua->request($req);
    if ($expected_code eq '204') {
        is($res->code, 204, "delete test lnp numbers");
        _check_lnpnumber_history("test lnp numbers are not found: ",$number,[],undef);
    } else {
