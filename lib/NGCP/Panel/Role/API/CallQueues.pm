        ...
        unless ($callqueue_uuids) {
            ...
            do {
                ...
                foreach my $mapkey (@$mapkeys) {
                    push(@$callqueue_uuids,$mapkey =~ s/^$redis_key_prefix//r);
                }
            } while ($cursor);
            $c->stash(callqueue_uuids => $callqueue_uuids);
        }
...
sub resource_from_item {
    ...
    #$c->config->{redis}->{callqueue_db} = 3;
    my $redis = $self->_get_redis($c,$c->config->{redis}->{callqueue_db});
    my $res = $redis->lrange($redis_key_prefix . $item->uuid, 0, -1);
    my @callqueue = ();
    if ($res) {
        foreach my $call_id (@$res) {
            my $queue_item = {};
            $queue_item->{call_id} = $call_id;
            # todo: fetch details like numbers/sip uri's ...
            # $self->_get_redis($c,'4')->lrange($call_id, 0 , -1)
