...
sub item_by_id {
    my ($self, $c, $id, $customer, $include_terminated,$now) = @_;

    my $item = $c->model('DB')->resultset('contract_fraud_preferences')->find({ contract_id => $id });
    unless ($item) {
        $customer //= $self->_item_rs($c,$include_terminated,$now)->find($id);
        if ($customer) {
...
sub update_item {
    ...
    try {
        ...
        $self->error($c, HTTP_INTERNAL_SERVER_ERROR, "Failed to create or update customer fraud preference.");
        return;
    };
