package NGCP::Panel::Controller::API::CallForwardsDNDItem;
...
use parent qw/Catalyst::Controller NGCP::Panel::Role::API::CallForwardsDND/;

sub resource_name{
    return 'callforwardsdnd';
}
sub dispatch_path{
    return '/api/callforwardsdnd/';
}
sub relation{
    return 'http://purl.org/sipwise/ngcp-api/#rel-callforwardsdnd';
}
...
sub PUT :Allow {
    ...
    my $guard = $c->model('DB')->txn_scope_guard;
    {
        ...
        unless ($item) {
            $self->error($c, HTTP_INTERNAL_SERVER_ERROR, "Failed to update callforward dnd.");
            last;
        }
...
sub DELETE :Allow {
    ...
    my $guard = $c->model('DB')->txn_scope_guard;
    {
        ...
        try {
            ...
            $c->log->error("Failed to delete callforward dnd with id '$id': $e");
