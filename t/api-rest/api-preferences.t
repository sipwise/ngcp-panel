...
foreach my $api (@apis){
    ...
    foreach my $preference_name(keys %{$defs->{content}}){
        ...
        if('boolean' eq $preference->{data_type}){
            ...
            $value = get_preference_id_value($preference) // "test_api preference string";
        }elsif('int' eq $preference->{data_type}){
            $value = get_preference_id_value($preference) // 33;
        }else{
        ...
        if($value && 'no_process' ne $value){
            ...
            print "Unprocessed preference: $api:$preference_name;\n";
...
sub get_preference_id_value{
    ...
    if($preference->{name}=~/^rewrite_rule_set$/){
        $res = $test_machine->{DATA_ITEM}->{rewriteruleset_id};
    }elsif($preference->{name}=~/^(adm_)?ncos$/){
        $res = $test_machine->{DATA_ITEM}->{ncoslevel_id};
    }elsif($preference->{name}=~/^(contract_)?sound_set$/){
        $res = $test_machine->{DATA_ITEM}->{soundset_id};
