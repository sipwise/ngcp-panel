...
sub DELETE :Allow {
    ...
    my $guard = $c->model('DB')->txn_scope_guard;
    {
        ...
        my $items = $self->item_rs($c,
            $c->request->params->{'actual'},
            $c->request->params->{'number'})->search(undef, {
                order_by => "me.id", # deadlock prevention
            });
        my $count = 0;
        #$items->delete; # delete_all might be more deadlock safe but triggers
        #                # the lnp_numbers_actual query after each delete
        for my $item ($items->all) {
            $c->log->debug("LNP number deleted: " . $item->id);
            $item->delete();
            $count++;
        }
        if ($count > 0) {
