...

{
    ...
    my $voucher = _create_voucher(10,'topup_start_mode_test1'.$t,$customer);
...
sub _check_topup_log {
    ...
    my ($customer,$expected_topup_log,$filter_query) = @_;
    my $total_count = (scalar @$expected_topup_log);
    my $i = 0;
    my $nexturi = $uri.'/api/topuplogs/?page=1&rows=10&order_by_direction=asc&order_by=timestamp'.$filter_query;
    do {
        $req = HTTP::Request->new('GET',$nexturi);
        $res = $ua->request($req);        
        #$res = $ua->get($nexturi);
        is($res->code, 200, "fetch topup log collection page");
        my $collection = JSON::from_json($res->decoded_content);
        my $selfuri = $uri . $collection->{_links}->{self}->{href};
        is($selfuri, $nexturi, "check _links.self.href of collection");
        my $colluri = URI->new($selfuri);

        ok($collection->{total_count} == $total_count, "check 'total_count' of collection");
        ...
        foreach my $log_record (@{ $collection->{_embedded}->{'ngcp:topuplogs'} }) {
            _compare_log_record($log_record,$expected_topup_log->[$i]);
...
sub _create_customer {
    ...
    $res = $ua->request($req);
    my $label = 'test customer with package ' . $package->{name};
