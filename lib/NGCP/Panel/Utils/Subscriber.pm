...
sub delete_callrecording {
    my %params = @_;
    my($recording, $force_files_deletion) = @params{qw/recording force_files_deletion/};
    ...
    foreach my $stream($recording->recording_streams->all) {
        ...
        if (! -e $stream->full_filename) {
            if ( !$force_files_deletion ) {
                die("Callrecording file ".$stream->full_filename." is absent");
            }
        } elsif( !unlink($stream->full_filename) && !$force_files_deletion ) {
            die($!);
        }
    }
