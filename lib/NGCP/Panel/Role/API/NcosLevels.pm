...
sub get_form {
    my ($self, $c) = @_;
    if($c->user->roles eq "admin") {
        require NGCP::Panel::Form::NCOS::AdminLevelAPI;
        return NGCP::Panel::Form::NCOS::AdminLevelAPI->new(ctx => $c);
    } elsif($c->user->roles eq "reseller") {
        require NGCP::Panel::Form::NCOS::ResellerLevelAPI;
...
sub hal_from_item {
    ...
    my $hal = NGCP::Panel::Utils::DataHal->new(
        links => [
            ...
            NGCP::Panel::Utils::DataHalLink->new(relation => 'ngcp:ncospatterns', href => sprintf("/api/ncospatterns/?ncos_level_id=%d", $item->id)),
        ],
