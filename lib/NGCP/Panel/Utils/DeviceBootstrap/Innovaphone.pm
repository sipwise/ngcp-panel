...
sub rpc_server_params{
    ...
    my $cfg  = {
        proto    => 'https',
        host     => 'my.innovaphone.com',
        port     => '443',
        path     => '/redirect/xmlrpc',
    };
    #$cfg->{headers} = { %{$self->get_basic_authorization($self->params->{credentials})} };
    --innovaphone-user='office@sipwise.com' --innovaphone-password='innova4Sipwise\!'
    $cfg->{headers} = { %{$self->get_basic_authorization( {user => 'office@sipwise.com', password => 'innova4Sipwise!' } )} };
