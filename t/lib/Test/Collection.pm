...
sub get_uri_item{
    ...
    if($item && 'HASH' eq ref $item && exists $item->{location}){ 
        $resuri = $self->normalize_uri('/'.$item->{location});
    }else{
        my @caller = caller(1);
        if ($caller[3] eq 'Test::Collection::get_uri_current'){
            @caller = caller(2);
        }
        if ($caller[3] eq 'Test::Collection::normalize_uri'){
            @caller = caller(3);
        }
        my $longmsg = longmess( "check_item_uri" );
        #print Dumper \@caller;
        #print $longmsg;
        if(!$self->ALLOW_EMPTY_COLLECTION){
            #print Dumper \@caller;
            #print $longmsg;
            die "No item found for the collection ".$self->name." and ALLOW_EMPTY_COLLECTION setting is OFF;\n";
        }else{
            print "No item found for the collection ".$self->name." and ALLOW_EMPTY_COLLECTION setting is ON;\n";;
        }
    }
    return $resuri;
}
